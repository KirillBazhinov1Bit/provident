<?require_once($_SERVER["DOCUMENT_ROOT"] . '/teams_section/md_team/header.php');?>
<?php
...
$ui_filter = [
	...
	['id' => 'DATE_CREATE', 'name' => 'Created on', 'type'=>'date', 'default' => true],
];
foreach($fieldsEntity as $key=>$value){
...
?>
	...
	<div class="filter_panel">
		<div>
			<a target="_blank" href="/crm/lead/details/0/"><button class="ui-btn ui-btn-success">Create</button></a>
		</div>
		<div>
...
if(isset($filterData['DATE_CREATE_from']) && $filterData['DATE_CREATE_from']){
	$filterData['>=DATE_CREATE'] = Bitrix\Main\Type\DateTime::createFromTimestamp(strtotime($filterData['DATE_CREATE_from']));
}
if(isset($filterData['DATE_CREATE_to']) && $filterData['DATE_CREATE_to']){
	$filterData['<=DATE_CREATE'] = Bitrix\Main\Type\DateTime::createFromTimestamp(strtotime($filterData['DATE_CREATE_to']));
}
//ID
...
foreach ($filterData as $k => $v) {
	if(in_array($k, ['DATE_CREATE', '<=DATE_CREATE', '>=DATE_CREATE', '<=ID', '>=ID'])) break;
			...
			switch($fieldsEntity[$id_field]['TYPE']){
				...
				default:
					if(is_array($value_field)){
						...
						if($id_field == 'TITLE'){
							$data[$id_field] = '<a href="/crm/lead/details/'.$item['ID'].'/">'.$value_field.'</a>';
						}else{
							$data[$id_field] = $value_field;
						}
					}
...
<?require_once($_SERVER["DOCUMENT_ROOT"] . '/teams_section/md_team/footer.php');?>