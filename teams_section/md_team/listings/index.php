...
$factory = \Bitrix\Crm\Service\Container::getInstance()->getFactory(LISTING_SPA_ID);
...
?>
	...
	<div class="filter_panel">
		<div>
			<a target="_blank" href="/crm/type/<?=LISTING_SPA_ID?>/details/0/"><button class="ui-btn ui-btn-success">Create</button></a>
		</div>
		<div>
			...
			switch($fieldsEntity[$id_field]['TYPE']){
				...
				default:
					if(is_array($value_field)){
						...
						if($id_field == 'TITLE'){
							$data[$id_field] = '<a href="/crm/type/'.LISTING_SPA_ID.'/details/'.$item['ID'].'/">'.$value_field.'</a>';
						}else{
							$data[$id_field] = $value_field;
						}
					}
